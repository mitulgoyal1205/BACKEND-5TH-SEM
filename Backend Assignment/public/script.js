...
const clearBtn = document.getElementById("clear-btn"); // added the button id to the clear button

getAllTodos(); // get todos and renders it when js loads

todoContainer.addEventListener("click", async (e) => {
  const btnClass = e.target.className;
  if (btnClass != "delete" && btnClass != "status") return; // if the button is not delete or status, then return
...
// Clear completed todos functionality this is what happens when the clear completed button is clicked
clearBtn.addEventListener("click", async () => {
  try {
    const res = await axios.delete("http://localhost:4000/todo/clear-completed");
